...
export default function NGOContent() {
  ...
  const [ngoBounties, setNgoBounties] = useState<Bounty[]>([]);
  const [ngoMilestones, setNgoMilestones] = useState<Record<number, Milestone[]>>({});
  const [ngoApplications, setNgoApplications] = useState<Record<number, Application[]>>({});
  ...
  const registerNGO = async () => {
    ...
    try {
      ...
      const tx = await registry.registerNGO(cid, payloadHash);
      await tx.wait();

      alert(`✅ NGO registered. CID: ${cid}`);
  ...
  const approveMilestone = async (bountyId: number, idx: number) => {
  ...
  const releaseMilestoneFunds = async (bountyId: number, idx: number) => {
  ...
  /*** FETCH NGO DATA ***/
  const fetchNGOData = async (registryContract: any, addr: string) => {
    const allBounties: Bounty[] = await registryContract.getAllBounties();
    ...
    <div className="admin-container">
      ...
      <section className="card">
        ...
        {cid && (
          <p>
            Pinned CID:{" "}
            <a href={`${PINATA_GATEWAY}/${cid}`} target="_blank" rel="noreferrer">
              {cid}
            </a>
          </p>
        )}
      ...
      <section className="card">
        ...
        <button onClick={createBounty} className="btn-accent">
          Create Bounty
        </button>
      </section>
      ...
      <section className="card">
        ...
        {ngoBounties.map((b) => (
          <div key={b.id} className="bounty-card">
            ...
            <h4>Milestones</h4>
            {ngoMilestones[b.id]?.map((m: Milestone, idx: number) => (
            ...
            {ngoApplications[b.id]?.map((app: Application, idx: number) => (
              <p key={idx}>
